  ...
  "The handler" should {
    ...
    "move a new local file to the correct directory" in {
      ...
      for {
        ...
        val actualMovedFilePath = Await.result(handler.processFoundDocument(foundDoc, s"$ingressDir/$sourceFile", handler.getLocalUpdateTargetPath(foundDoc), handler.inLocalRoot(foundDoc.doc.source)), 5.seconds)
    ...
    "move a new remote https file to the correct directory" in {
      ...
      for {
        ...
        val actualMovedFilePath = Await.result(handler.processFoundDocument(foundDoc, s"$remoteIngressDir/$sourceFile", handler.getRemoteUpdateTargetPath(foundDoc), handler.inRemoteRoot(foundDoc.doc.source)), 5.seconds)
