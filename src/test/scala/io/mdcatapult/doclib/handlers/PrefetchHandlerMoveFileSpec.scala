  ...
  "The handler" should {
    ...
    "move a new local file to the correct directory" in {
      ...
      for {
        ...
        val actualMovedFilePath = Await.result(handler.processFoundDocument(foundDoc, s"$ingressDir/$sourceFile", handler.getLocalUpdateTargetPath, handler.inLocalRoot), 5.seconds)
    ...
    "move a new remote https file to the correct directory" in {
      ...
      for {
        ...
        val actualMovedFilePath = Await.result(handler.processFoundDocument(foundDoc, s"$remoteIngressDir/$sourceFile", handler.getRemoteUpdateTargetPath, handler.inRemoteRoot), 5.seconds)
