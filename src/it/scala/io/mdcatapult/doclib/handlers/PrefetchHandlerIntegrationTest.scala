...
class PrefetchHandlerIntegrationTests extends TestKit(ActorSystem("PrefetchHandlerIntegrationTest", ConfigFactory.parseString(
  ...
  "A parent doc " should {
    "be updated wth new child info" in {
      ...
      val parentResult = Await.result(collection.insertOne(parentDoc).toFutureOption(), 5 seconds)

      assert(parentResult.get.toString == "The operation completed successfully")
      ...
      val origin: Origin = Origin(
        ...
        metadata = Some(List(MetaString("_id", parentId.toString))),
        headers = None
      )
      val prefetchMsg: PrefetchMsg = PrefetchMsg("ingress/derivatives/remote/http/path/to/unarchived_parent.zip/child.txt", Some(List(origin)), Some(List("a-tag")), Some(metadataMap), Some(true))
    ...
    //      val filter = and(mequal("_id", parentId), exists("derivatives.path"), mequal("derivatives.path", "ingress/derivatives/remote/http/path/to/unarchived_parent.zip/child.txt"))
    //      val pullDerivative = pull("derivatives", mequal("path", "ingress/derivatives/remote/http/path/to/unarchived_parent.zip/child.txt"))
    //      val pushDerivative = addToSet("derivatives", newDerivative)
    //      val update = combine(pullDerivative, pushDerivative)
    //      collection.updateOne(filter, update).toFutureOption().andThen({
    ...
    //  "A parent doc " should {
    //    "be updated wth new child info" in {
    //      val parentId = new ObjectId().toString
    //      val metadataMap: List[MetaString] = List(MetaString("doi", "10.1101/327015"))
    //      val origin: Origin = Origin(
    //        scheme = "https",
    //        uri = Some(Uri.parse("https:/parent")),
    //        metadata = Some(List(MetaString("_id", parentId))),
    //        headers = None
    //      )
    //
